...
def get_sfr_control_rows(session):
    ...
    for person in persons:
        if person.linked_defenders:  # если с персоной связан защитник
            ...
            answers = []
            if defenders:
                answers = get_control_answers(defenders)
...
def get_control_answers(defenders):
    # результат
    result = {
        'doc_type': '',
        'doc_ser': '',
...
def get_sfr_forgiven_rows(session):
    rows = []
    row_number = 0
    persons = get_persons(session)
    for person in persons:
        if person.linked_defenders:  # если с персоной связан защитник
            ...
            answers = []
            if defenders:
                answers = get_forgiven_answers(defenders)
            # если ответы есть, добавляем строку
            if answers:
                row_number += 1
                rows.append([row_number] + person_data + answers)
...
def get_forgiven_answers(defenders):
    # результат
    result = {
        'doc_type': '',
        'doc_ser': '',
    ...
    if (
            result['answer'] and
            'загружены' not in result['answer'] and
            'добавлен' not in result['answer']
    ):
        return [
            result['doc_type'],
            result['doc_ser'],

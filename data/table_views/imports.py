...
from modules.exporter import defenders_load_data, defenders_identify_data
from tools import datetime_to_string, write_xls_list, File, Directory
...
class ImportUI:
    ...
    def create_init_protocol(self):
        ...
        # filename = self._app_.storage.imports.subject_directory(self.model.eskk_military_subject_id)
    ...
    def table_view_import(self):
        return [
            datetime_to_string(self.model.created_utc) if self.model else None,
            self.model.id if self.model else None,
            self.model.eskk_military_subject.short_name if self.model else None,
            self.record_count.total,
            self.record_count.on_send,
...
class ImportsUI:
    ...
    def total_row(self):
        return [
            'ИТОГО',
            '{} дня(ей)'.format((self.period['max'] - self.period['min']).days),

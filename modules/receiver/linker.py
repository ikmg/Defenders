...
from . import PickedSNILSHandler, PickedLastNameHandler, PickedFirstNameHandler, PickedMiddleNameHandler
from . import PickedDocumentSerialHandler, PickedDocumentNumberHandler, PickedDocumentOrganizationHandler
from . import (PickedAddressIndexHandler, PickedAddressRegionHandler, PickedAddressAreaHandler, PickedAddressLocalityHandler,
               PickedAddressStreetHandler, PickedAddressHouseHandler, PickedAddressBuildingHandler, PickedAddressFlatHandler)
from . import PickedMilitaryRankHandler, PickedPersonalNumberHandler, PickedMilitarySubjectHandler

from tools.strings import clear_string, get_fio_list
from tools.date_and_time import date_to_string, string_to_date, datetime_to_string
...
class LinkedPersonHandler(LinkerHandler):
    ...
    def birthday_date(self):
        return string_to_date(self.birthday)
    ...
    def check(self):
        ...
        if self.birthday:
            if self.birthday_date:
                if self.age(18) > datetime.now().date():
        ...
        if self.is_already_exists:
            self.warning_messages.append('<Персона> уже идентифицировалась {}'.format(datetime_to_string(self.created_utc)))
        # сбор всех ошибок
...
class LinkedDocumentHandler(LinkerHandler):
    ...
    def check(self):
        ...
        if self.date:
            if self.document_date:
                if self.document_date > datetime.now().date():
...
class LinkedDocumentVBDHandler(LinkerHandler):
    ...
    def check(self):
        ...
        if self.date:
            if self.document_date:
                ...
                elif self.document_date < string_to_date('01.01.2004'):
...
class LinkedAddressHandler(LinkerHandler):
...
class LinkedOrderFIOHandler(LinkerHandler):
    ...
    def __init__(self, session, value):
        ...
        # if 'ё' in self.picked_middle_name.value or 'ё' in self.picked_middle_name.value:
        #     print(self.picked_middle_name.value)
        self.get_model()
        self.check()
...
class LinkedOrderPersonHandler(LinkerHandler):
...
class LinkedDefenderHandler(LinkerHandler):
    ...
    def exclude_real_date(self):
        return string_to_date(self.exclude_date)
    ...
    def check(self):
        ...
        if self.exclude_date:
            if self.exclude_real_date:
                if self.exclude_real_date > datetime.now().date():
        ...
        if self.linked_document.eskk_document_type_id == '21':
            if self.linked_document.document_date and self.linked_person.age(20) and self.linked_document.document_date < self.linked_person.age(20) < datetime.now().date():
                self.warning_messages.append('<Паспорт> требует замены по достижению возраста 20 лет')
            if self.linked_document.document_date and self.linked_person.age(45) and self.linked_document.document_date < self.linked_person.age(45) < datetime.now().date():
...
class LinkedOrderPersonPeriodHandler(LinkerHandler):
    ...
    def __init__(self, session, **keywords):
        ...
        self.begin_list = clear_string(date_to_string(keywords['date_begin'])).split(' ')
        self.end_list = clear_string(date_to_string(keywords['date_end'])).split(' ')
    ...
    def date_begin_real(self):
        return string_to_date(self.date_begin)

    @property
    def date_end_real(self):
        return string_to_date(self.date_end)
        ...
        if len(self.begin_list) != len(self.end_list):
            ...
            for index in range(0, len(self.begin_list)):
                ...
                if not self.is_already_exists:
                    ...
                    self.model.created_utc = datetime.now()

...
class DialogImport(QDialog, Ui_Dialog_import):
    ...
    def __init__(self, app: DefendersApp, import_item: ImportData):
        self.app = app
    ...
    def set_buttons_enabled(self):
        if self.import_item.model.is_finished:
        ...
        if not self.import_item.is_can_make_result:
            self.pushButton_import_finish.setEnabled(False)

        if not self.import_item.is_can_delete:
            self.pushButton_import_delete.setEnabled(False)
    ...
    def edit_contacts(self):
        if self.lineEdit_contacts.text() != self.import_item.model.contact_info:
            msg = QMessageBox(self)
            msg.setWindowTitle('Изменение контакта')
            msg.setIcon(QMessageBox.Question)
            msg.setText('Контактные данные были изменены.\n\nСохранить изменения?'.format(
                self.import_item.model.id,
                self.import_item.model.eskk_military_subject.short_name))
    ...
    def delete_import(self):
        ...
        if msg.clickedButton() == button_yes:
            try:
                ...
                QMessageBox.information(self, 'Удаление загрузки', 'Загрузка {} успешно удалена'.format(self.import_item.model.id))
            except Exception as e:
                QMessageBox.critical(self, 'Удаление загрузки', 'Не удалось удалить загрузку {} ({})'.format(self.import_item.model.id, e))
